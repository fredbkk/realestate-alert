...
BASE_URL = "https://www.swissproperty.ch/en/properties/search?location=bussigny"
SEEN_FILE = "seen_swissproperty.json"
...
def fetch():
    ...
    ads = soup.select("a.card-wrapper[href]")
    seen, new_ads = load_seen(), []
    for ad in ads:
        title = ad.get_text(strip=True).lower()
        href = ad.get("href")
        url = f"https://www.swissproperty.ch{href}"
        ad_id = href.split("/")[-1]
...
if __name__ == "__main__":
    log("🔎 Surveillance Swissproperty...")
    r = fetch()
