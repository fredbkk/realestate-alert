...
BASE_URL = "https://flatfox.ch/fr/search/?q=bussigny&type=house&offer=buy"
SEEN_FILE = "seen_flatfox.json"
...
def fetch():
    ...
    ads = driver.find_elements(By.CSS_SELECTOR, "a.result-list__item-link")
    seen, new_ads = load_seen(), []
    for ad in ads:
        title = ad.text.strip().lower()
        url = ad.get_attribute("href"); ad_id = url.split("/")[-2]
