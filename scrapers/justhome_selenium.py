...
BASE_URL = "https://justhome.ch/buy?location=bussigny&type=house"
SEEN_FILE = "seen_justhome.json"
...
def fetch():
    ...
    ads = driver.find_elements(By.CSS_SELECTOR, "a[href^='/buy/property']")
    ...
    for ad in ads:
        ...
        if ad_id not in seen and any(k in title for k in KEYWORDS):
            new_ads.append({"title": title, "url": f"https://justhome.ch{url}"}); seen.add(ad_id)
    driver.quit(); save_seen(seen)
    if new_ads: json.dump(new_ads, open(OUTPUT, "w"))
    return new_ads

if __name__ == "__main__":
    log("🧭 Surveillance JustHome...")
    r = fetch()
